...
public class CharacterCreationReviewStageView : CharacterCreationStageViewBase
{
		...
		: base(affirmativeAction, negativeAction, onRefresh, getCurrentStageIndexAction, getTotalStageCountAction, getFurthestIndexAction, goToIndexAction)
	{
		...
		GauntletLayer = new GauntletLayer("CharacterCreationReview", 1);
		GauntletLayer.InputRestrictions.SetInputRestrictions();
		GauntletLayer.IsFocusLayer = true;
		GauntletLayer.Input.RegisterHotKeyCategory(HotKeyManager.GetCategory("GenericPanelGameKeyCategory"));
		ScreenManager.TrySetFocus(GauntletLayer);
		bool isBannerAndClanNameSet = _characterCreationManager.GetStage<CharacterCreationBannerEditorStage>() != null && _characterCreationManager.GetStage<CharacterCreationClanNamingStage>() != null;
		_dataSource = new CharacterCreationReviewStageVM(_characterCreationManager, NextStage, _affirmativeActionText, PreviousStage, _negativeActionText, isBannerAndClanNameSet);
	...
	private void AddCharacterEntity()
	{
		...
		_agentVisuals = AgentVisuals.Create(agentVisualsData, "facegenvisual", isRandomProgress: false, needBatchedVersionForWeaponMeshes: false, forceUseFaceCache: true);
	...
	private void RefreshMountEntity()
	{
		...
		if (CharacterObject.PlayerCharacter.HasMount())
		{
			...
			_mountEntity.Skeleton.SetAgentActionChannel(0, in ActionIndexCache.act_inventory_idle_start);
