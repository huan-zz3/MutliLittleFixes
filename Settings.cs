...
namespace MutliLittleFixes
{
    internal sealed class Settings : AttributeGlobalSettings<Settings>
    {
        ...
        */
        ...
        [SettingPropertyBool("{=mlf_autoresolve_battlelog}Auto Resolve Battle Log (CSV)", Order = 98, RequireRestart = false, HintText = "{=mlf_autoresolve_battlelog_hint}Write a full CSV log of every player-simulated auto-resolve battle for later analysis (battle summary, per-round stats, per-hit weapon/armor/damage details, and every casualty event). Files are saved to a timestamped folder under 'Documents\\Mount and Blade II Bannerlord\\MutliLittleFixes_AutoResolveLogs\\' (the game's standard user folder, same as rgl_log.txt / Configs) with UTF-8 BOM encoding and can be opened directly in Excel/WPS. Only battles simulated by the player are logged; AI vs AI battles are never recorded")]
        [SettingPropertyGroup("{=mlf_group_autoresolve}Auto Resolve Rebalance")]
