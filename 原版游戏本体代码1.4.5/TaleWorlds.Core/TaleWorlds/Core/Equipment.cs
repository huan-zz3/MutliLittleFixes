...
public class Equipment
{
	...
	public void DeserializeNode(MBObjectManager objectManager, XmlNode node)
	{
		if (node.NodeType != XmlNodeType.Comment)
		{
			...
			{
				Debug.FailedAssert(string.Concat((itemObject == null) ? TextObject.GetEmpty() : itemObject.Name, " does not fit to slot ", equipmentIndexFromOldEquipmentIndexName), "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.Core\\Equipment.cs", "DeserializeNode", 171);
			}
		}
	}
	...
	public bool HasWeapon()
	{
		for (int i = 0; i < 5; i++)
		{
			...
			if (!equipmentElement.IsEmpty && equipmentElement.Item.ItemType != ItemObject.ItemTypeEnum.Invalid && equipmentElement.Item.PrimaryWeapon.WeaponFlags.HasAnyFlag(WeaponFlags.WeaponMask))
			{
				return true;
			}
		}
