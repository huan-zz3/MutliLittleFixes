...
- Current version: v1.1.0 (see changelog at the end)
...
- **New in v1.1.0:** Grain transport support between player fiefs; exiled clans never die out & encyclopedia exile filter; player's party not handed to AI on death.
...
- **Exiled Clans Never Die Out:** Removes the 28-day survival countdown extinction mechanic for landless exiled clans (clans wandering after their kingdom is destroyed), letting them persist forever until they join another kingdom or gain territory; when disabled, the vanilla countdown extinction is restored. (Off by default)
...
- **No Over-Capacity Slowdown for Grain Convoys:** Grain convoys have no hero leading them; vanilla gives them a base party cap of only 20, so exceeding it triggers a slowdown penalty. This feature raises the convoy party cap to the configured "convoy size" value, eliminating the over-capacity slowdown so grain arrives faster.
...
- New feature: Grain support between player fiefs (usable) — surplus towns of the player's clan automatically send garrison-converted grain convoys to grain-short towns/castles; grain is added/deducted directly without going through the market, bundled with the "No Over-Capacity Slowdown for Grain Convoys" optimization.
...
- New feature: Encyclopedia clan exile filter (usable) — the "Status" filter group on the clan list gains "In Exile / Not in Exile" filter options.
- New feature: Exiled clans never die out (untested, off by default) — removes the 28-day survival countdown extinction for exiled clans, letting them persist forever.
- New feature: No party takeover on player death (untested, off by default) — the party keeps fighting with its last order after the player dies, not taken over by AI.
